...
if (pdfContainer) {
  ...
  // Der touchend-Listener empfängt jetzt das 'event'-Objekt
  pdfContainer.addEventListener('touchend', function(event) { // <--- 'event' hier hinzugefügt
    const currentTime = new Date().getTime();
    const tapLength = currentTime - lastTap;
    lastTap = currentTime;

    // ===================================================================
    //   NEU: DOPPELTIPP-ERKENNUNG MIT VERHINDERUNG DES STANDARD-ZOOMS
    // ===================================================================
    if (tapLength < 300 && tapLength > 0 && Math.abs(distanzX) < 20) {
      
      // 1. ZUERST das Standard-Browser-Zoomen für diesen Doppeltipp verhindern
      event.preventDefault(); // <--- DAS IST DER ENTSCHEIDENDE BEFEHL

      // 2. DANN unsere eigene Aktion ausführen
      if (currentDocumentPath && pdfDoc) {
        const urlForNewTab = `${currentDocumentPath}#page=${currentPage}`;
        window.open(urlForNewTab, '_blank');
      }
      
      // 3. Die Funktion beenden
      return; 
    }
